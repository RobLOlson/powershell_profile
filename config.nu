...
# Default: []
# $env.config.hooks.pre_prompt = [ (term size).columns | save -f 'C:\users\sterl\.config\columns.txt']
...
$env.__zoxide_hooked = null
source ~/.zoxide.nu


# custom wrapper around z so that calling without arguments goes directly into interactive mode
...
# These lines fixed an abberant emoji from showing up in shell, but then the problem went away...
